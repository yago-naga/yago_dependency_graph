...
public class RunExtractor extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    String extractorName = request.getParameter("extractorName") != null ? request.getParameter("extractorName") : "";
  ...
  protected void doThemeFiltering(String filteredTheme, Set<String> patternThemeSet) {
    ...
    D.p(String.format("Copying from %s/%s.ttl to %s/%s_orig.ttl", Parameters.get("yagoFolder"), filteredTheme, Parameters.get("yagoFolder"),
        filteredTheme));
    ...
    for (String patternTheme : patternThemeSet) {
      ...
      String[] awk = {
          ...
          "-c",
          String.format(
              "awk '{ if (match($1, /^</)) print substr($1, index($1,\"<\") + 1, index($1,\">\") - 2); if (match($1, /^</) && match($3, /^</)) print substr($3, index($3,\"<\") + 1, index($3,\">\") - 2); }' %s/%s.ttl "
                  + "| sort -u > %s/%s_tmp.ttl",
      ...
      String[] sed = { "/bin/sh", "-c", String.format("cat %s/%s_tmp.ttl | sed 's/\\_/ /g' >> %s/%s_tmp.ttl", dir, patternTheme, dir, patternTheme) };
      ...
      String[] fgrep = {
          "/bin/sh",
          "-c",
          String.format("fgrep -f %s/%s_tmp.ttl %s/%s_orig.ttl >> %s/%s.ttl; rm %s/%s_tmp.ttl", dir, patternTheme, dir, filteredTheme, dir,
              filteredTheme, dir, patternTheme) };
