...
public class MakeSvg {
  ...
  protected static final String boilerplate = "<svg xmlns='http://www.w3.org/2000/svg'\n" + "    xmlns:xlink='http://www.w3.org/1999/xlink'\n"
      + "    width='"
      + windowWidth
      + "' height='@HEIGHT' preserveAspectRation='MidX' font-size='"
      + fontSize
      + "' font-family='Arial'>\n"
      + "  <defs>\n"
      + "    <marker id='arrow'\n"
      ...
      + "      orient='auto'>\n"
      + "       <path d='M 0 0 L 10 5 L 0 10 z' stroke='"
      + lineColour
      + "' fill='"
      + lineColour
      + "'/>\n"
  ...
  public static List<Extractor> nextLine(List<Extractor> extractors, Set<Extractor> extractorsWeHave, Map<Theme, Pair<Integer, Integer>> themesWeHave) {
  ...
  public static StringBuilder makeSvg(List<Extractor> extractors, File yagoFolder) {
    ...
    while (true) {
      ...
      for (Extractor extractor : nextline) {
        ...
        for (Theme theme : extractor.input()) {
          ...
          if (y - themesWeHave.get(theme).second() > themeGap + themeHeight) {
            line(line, lineColour, themesWeHave.get(theme).first(), themesWeHave.get(theme).second(), x, y - extractorHeight / 2 - 2, 2, 0.1,
                ...
                "extractor-id", getElementId(extractor.name(), x, y),
                "dependencies", seq(themeDependencies.get(theme)));
          } else {
        ...
        if (extractor.inputDataFile() != null) {
        	inputThemes += inputThemes.isEmpty() ? extractor.inputDataFile().getName() : ";" + extractor.inputDataFile().getName();
        }
        ellipse(result, extractorColour, lineColour, 2, x, y, extractorWidth / 2, extractorHeight / 2, 
        		"input-themes", inputThemes,
        		"output-themes", seq(extractor.output()),
        		"extractor-name", extractor.name(), 
        		"extractor-id", getElementId(extractor.name(), x, y),
        		"dependencies", seq(extractorDependencies),
        		"dependency-id", extractorCount.toString());
        ...
        if (extractor.inputDataFile() != null) {
          ...
          name = extractor.inputDataFile().getName();
          if (extractor.inputDataFile().isDirectory()) {
            name = "Folder: " + name;
          }
          int inputBoxWidth = fontSize * name.length() * 60 / 100;
          rect(result, dataColour, "black", 1, inputX, inputY, inputBoxWidth, sourceHeight, 
        		  "data-file", extractor.inputDataFile().isDirectory() ? "false" : "true", 
        		  "theme-name", name);
          text(result, "black", inputX + fontSize / 4, inputY + fontSize, name, 
        		  "theme-name", name,
        		  "data-file", extractor.inputDataFile().isDirectory() ? "false" : "true");
          result.append("  </g>\n");
        ...
        if (!extractor.output().isEmpty()) {
          for (Theme theme : extractor.output()) {
            ...
            text(result, "black", themeX + fontSize / 4, themeY + fontSize, name.substring(0, charsPerLine), "theme-name", theme.name, "dependencies", seq(extractorDependencies));
            text(result, "black", themeX + fontSize / 4, themeY + fontSize * 2, name.substring(charsPerLine, charsPerLine * 2), "theme-name",
                theme.name, "dependencies", seq(extractorDependencies));
            text(result, "black", themeX + fontSize / 4, themeY + fontSize * 3, name.substring(charsPerLine * 2), "theme-name", theme.name, "dependencies", seq(extractorDependencies));
            result.append("  </g>");
  ...
  public static void main(String[] args) throws Exception {
    ...
    makeSvg(extractors(new File(initFile)), new File("/home/jbiega/data/yago_tmp"), new File("/home/jbiega/data/glass2.svg"));
  }
}
